...
class SiteInfo(BaseClass):
    ...
    Args:
        ...
        wind_resource_files: List of paths to wind resource files. Defaults to [].
    ...
    wind_resource_files: List[Union[Path, str]] = field(default=Factory(list), converter=lambda x: [resource_file_converter(f) for f in x] if isinstance(x, list) else [resource_file_converter(x)] if x else [])
    ...
    wind_sub_resources: List[Union[WindResource, HPCWindData]] = field(init=False, default=Factory(list))
    ...
    def __attrs_post_init__(self):
        ...
        if self.wind:
            ...
            main_resource = self.initialize_wind_resource(data)
            self.wind_resource = main_resource

            #check if the main resource has the same timesteps as the sub resources
            ...
            # check if the sub resources have the same timesteps as the main resource
            if self.wind_sub_resources:
                for i, sub_resource in enumerate(self.wind_sub_resources, 1):
                    sub_n_timesteps = len(sub_resource.data['data']) // 8760 * 8760
                    if self.n_timesteps != sub_n_timesteps:
                        raise ValueError(f"Sub wind resource {i} timesteps of {sub_n_timesteps} different than other resource timesteps of {self.n_timesteps}")
                    
            logger.info("Set up SiteInfo with wind resource file: {}".format(self.wind_resource.filename))
            if self.wind_sub_resources:
                logger.info(f"Set up SiteInfo with {len(self.wind_sub_resources)} additional wind resource files")
    ...
    def initialize_wind_resource(self,data:dict):
        ...
        if self.wind_resource is None:
            # main wind farm resource
            main_resource = None
            if self.wind_resource_files and self.wind_resource_files[0]:
                if self.renewable_resource_origin == "API":
                    main_resource = WindResource(wind_lat, wind_lon, wind_year, 
                                           wind_turbine_hub_ht=self.hub_height,
                                           path_resource=self.path_resource, 
                                           filepath=self.wind_resource_files[0], 
                                           source=self.wind_resource_origin)
            else:
                main_resource = HPCWindData(wind_lat, wind_lon, wind_year, 
                                          wind_turbine_hub_ht=self.hub_height,
                                          wtk_source_path=self.wtk_source_path, 
                                          filepath=self.wind_resource_files[0])
            sub_resources = []
            for i, file_path in enumerate(self.wind_resource_files[1:], 1):
                if file_path:
                    sub_lat = data.get(f"wind_lat_{i}", wind_lat)
                    sub_lon = data.get(f"wind_lon_{i}", wind_lon)
                    sub_year = data.get(f"wind_year_{i}", wind_year)
                
                    if self.renewable_resource_origin == "API":
                        sub_resource = WindResource(sub_lat, sub_lon, sub_year,
                                              wind_turbine_hub_ht=self.hub_height,
                                              path_resource=self.path_resource,
                                              filepath=file_path,
                                              source=self.wind_resource_origin)
                    else:
                        sub_resource = HPCWindData(sub_lat, sub_lon, sub_year,
                                             wind_turbine_hub_ht=self.hub_height,
                                             wtk_source_path=self.wtk_source_path,
                                             filepath=file_path)
                    sub_resources.append(sub_resource)
                
            # Save main resource and sub-resource list
            self.wind_sub_resources = sub_resources
