...
from hopp.simulation.technologies.wind.multi_wind_plant import MultiWindPlant, MultiWindConfig
...
class TechnologiesConfig(BaseClass):
    ...
    Args:
        ...
        wind: Wind config. Can be either WindConfig for single wind farm or MultiWindConfig for multiple wind farms
        wave: Wave config
        tidal: Tidal config
    ...
    wind: Optional[Union[WindConfig, MultiWindConfig]] = field(default=None)
    ...
    def from_dict(cls, data: dict):
        ...
        if "wind" in data:
            if "model_name" in data["wind"] and data["wind"]["model_name"] == "multi_wind":
                config["wind"] = MultiWindConfig.from_dict(data["wind"])
            else:
...
class HybridSimulation(BaseClass):
    ...
    wind: Optional[Union[WindPlant, MultiWindPlant]] = field(init=False, default=None)
    ...
    def __attrs_post_init__(self):
        ...
        if wind_config is not None:
            if isinstance(wind_config, MultiWindConfig):
                self.wind = MultiWindPlant(self.site, config=wind_config)    # Multi wind farm
            else:
                self.wind = WindPlant(self.site, config=wind_config)         # Single wind farm
